...
// console.log('controlDesc||contorlBlockName||controlOwnerName',controlDesc,contorlBlockName,controlOwnerName);
ncData.push({
"_id" : data[i]._id,
...
// res.status(200).json({"nc_true ":req.params.assessments_ID});
// console.log('corporate_ID ',req.params.corporate_ID);
Assessments.aggregate([
{
...
// res.status(200).json({"nc_true ":req.params.assessments_ID});
// console.log('assessedParty_ID ',req.params.assessedParty_ID);
Assessments.aggregate([
{
...
if(data[i].framework.nc.actionPlan&&data[i].framework.nc.actionPlan.length>0){
	for (var j = 0; j < data[i].framework.nc.actionPlan.length; j++) {
		// actionPlan.push(data[i].framework.nc.actionPlan[j])
		ncData.push({
		"_id" : data[i]._id,
		"control_ID" : data[i].framework.control_ID,
		"controlDesc" : controlDesc,
	...
	if(assessmentData.framework&&assessmentData.framework.length>0){
		for (var i = 0; i < assessmentData.framework.length; i++) {
			if(assessmentData.framework[i].controlBlock_ID==req.body.controlBlock_ID&&assessmentData.framework[i].control_ID==req.body.control_ID){
                ...
                console.log('count==(assessmentData.framework[i].nc.actionPlan.length-1)',count,(assessmentData.framework[i].nc.actionPlan.length-1));
